...
// 全局缓存组管理，该模块将缓存、分布式通信、数据加载策略融为一体
var (
	groupsMu sync.RWMutex
	groups   = make(map[string]*Group) // 名称到缓存组的映射
)
...
// Getter 加载键值的回调函数接口，接口定义了缓存未命中时从数据源加载数据的 Get 方法
...
func (g *Group) syncToPeers(ctx context.Context, op string, key string, value []byte) {
	...
	if err != nil {
		logrus.Errorf("[KakaCache] failed to sync %s to peer: %v", op, err)
	}
}
...
func (g *Group) Clear() {
	...
	g.mainCache.Clear()
	logrus.Infof("[KakaCache] cleared cache for group [%s]", g.name)
}
...
func (g *Group) Close() error {
	...
	logrus.Infof("[KakaCache] closed cache group [%s]", g.name)
	return nil
}
...
func (g *Group) loadData(ctx context.Context, key string) (value ByteView, err error) {
	...
	if g.peers != nil {
		...
		if ok && !isSelf {
			...
			logrus.Warnf("[KakaCache] failed to get from peer: %v", err)
		}
	}
...
func (g *Group) RegisterPeers(peers PeerPicker) {
	...
	g.peers = peers
	logrus.Infof("[KakaCache] registered peers for group [%s]", g.name)
}
...
func DestroyGroup(name string) bool {
	...
	if g, exists := groups[name]; exists {
		g.Close()
		delete(groups, name)
		logrus.Infof("[KakaCache] destroyed cache group [%s]", name)
		return true
	}
