...
func TestCacheAdjust(t *testing.T) {
	...
	// 将key1移动到链表头部
	c.adjust(idx1, prev, next)

	// 验证key1现在是最近使用的
	if c.dlnk[0][next] != idx1 {
		t.Errorf("Expected key1 to be at the head of the list")
	}

	// 将key1移动到链表尾部
	c.adjust(idx1, next, prev)

	// 验证key1现在是最少使用的
	if c.dlnk[0][prev] != idx1 {
...
func TestLRU2StoreLRUEviction(t *testing.T) {
	...
	// 在 key1 被挤出前，我们需要让它晋升到二级缓存
	// LRU-2 晋升机制：只有在 Get 命中时才会晋升
	store.Get("key1") // key1 现在晋升到二级缓存

	store.Set("key3", testValue("value3")) // key2 会在一级缓存被挤掉（或者如果是纯淘汰，这里看实现）
	...
	store.Get("key4") // 晋升 key4

	store.Set("key5", testValue("value5"))
	store.Get("key5") // 晋升 key5，这会导致 key1 从二级缓存中被淘汰（因为 Level2Cap 是 2，里面有 key1, key4, 塞入 key5 时最老的 key1 滚蛋）
...
func TestLRU2StoreHitRatio(t *testing.T) {
	...
	// 验证命中率大致为0.25-0.55（因为我们添加了50个项但有分桶和LRU淘汰）
	if hitRatio < 0.25 || hitRatio > 0.55 {
